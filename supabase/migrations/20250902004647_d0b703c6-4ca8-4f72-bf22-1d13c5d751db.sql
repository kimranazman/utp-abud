...
-- 3. Admins can view all profiles (already exists but ensure it's correct)
-- The existing "Admins can view all profiles" policy should remain

-- 4. Profile modification policies (keep existing)
-- The existing insert and update policies for users are fine
...
-- Add RLS policies for contribution_analytics view to fix the analytics exposure
...
-- The existing admin policy for contribution_analytics should remain

-- Add RLS policy for development_status view (admin only)
-- This was identified as another security issue
-- Note: Since development_status is a view without user-specific data, 
-- we'll restrict it to admins only